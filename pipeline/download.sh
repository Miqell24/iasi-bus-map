#!/usr/bin/env bash
# Downloads input data: Iași GTFS, OSM networks (Overpass), MapLibre GL.
# Everything is cached — re-running only fetches what is missing.
#
# ONE feed covers the whole network: Societatea de Transport Public Iași (SCTP)'s
# buses and trams, split by route_type at build time.
set -euo pipefail
cd "$(dirname "$0")/.."
mkdir -p data/gtfs data/osm web/vendor

BB=47.03,27.41,47.32,27.86

# 1) GTFS
if [ ! -f data/gtfs/routes.txt ]; then
  echo "== SCTP GTFS (Iași) =="
  curl -fL --retry 3 --max-time 600 -o data/iasi-gtfs.zip \
    "https://external.gtfs.ro/iasi/IASI.zip"
  unzip -o data/iasi-gtfs.zip -d data/gtfs
fi

# 2) OSM — roadways over the feed's extent plus margin.
if [ ! -f data/osm/iasi.json ]; then
  echo "== Overpass (roads) =="
  ...
  for EP in "https://overpass-api.de/api/interpreter" \
    ...
    echo "-- $EP"
    if curl -fsS --max-time 900 -o data/osm/iasi.json --data-urlencode "data=$QR" "$EP" \
       && grep -q '"elements"' data/osm/iasi.json; then
      ok=1; break
    fi
    sleep 5
  done
...
if [ ! -f data/osm/iasi-rail.json ]; then
  ...
  for EP in "https://overpass-api.de/api/interpreter" \
            ...
            "https://overpass.kumi.systems/api/interpreter"; do
    echo "-- $EP"
    if curl -fsS --max-time 300 -o data/osm/iasi-rail.json --data-urlencode "data=$QT" "$EP" \
       && grep -q '"elements"' data/osm/iasi-rail.json; then
      ok=1; break
    fi
    sleep 5
  done
...
echo "OK — data ready:"
du -sh data/iasi-gtfs.zip data/osm/iasi*.json 2>/dev/null || true
