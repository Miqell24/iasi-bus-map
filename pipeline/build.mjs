...
// Iași: ONE SCTP feed split by route_type — buses (route_type 3) on the
// road graph, trams (0) on the tram graph. There is no metro here, so the engine's metro
...
// ONE feed: routeTypes splits the SCTP bundle per mode
// (3 = bus;
// 0 = tram on the tram graph). Without the filter `--all` on the bus mode would
// swallow the tram lines too.
// Iași has no metro, so the third cfg its siblings carry is absent here.
const MODES = [{
  mode: 'bus', label: 'buses', osmFile: 'data/osm/iasi.json',
  graphMode: 'road', color: '#0059a9', colorDark: '#00294f',
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['28']),
  feeds: [
    {
      tag: 'iasi', dir: 'data/gtfs', routeTypes: ['3'],
      mapKey: (sn) => norm(sn),
    },
  ],
}];
...
if (tramAll || tramSel.length) MODES.push({
  mode: 'tram', label: 'trams', osmFile: 'data/osm/iasi-rail.json',
  ...
  feeds: [
    { tag: 'iasi', dir: 'data/gtfs', mapKey: (sn) => norm(sn), routeTypes: ['0'] },
  ],
});
